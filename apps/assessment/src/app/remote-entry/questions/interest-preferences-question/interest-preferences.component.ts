...
@Component({
  selector: 'app-interest-preferences',
  templateUrl: './interest-preferences.component.html',
  styleUrl: './interest-preferences.component.scss',
})
export class InterestPreferencesComponent implements OnInit, OnDestroy {
  @Input()
  question!: InterestAndPreferencesQuestionDto;
...
sanitizedQuestionText!: SafeHtml; // Variable to store sanitized HTML
  ...
  constructor(private readonly assessmentService: AssessmentService,private sanitizer: DomSanitizer, private renderer: Renderer2
  ) {}
  ngOnInit() {
    if (this.question && !(this.question.options?.length === 0)) {
      ...
      this.assessmentService
        .getInterestAndPreferencesQuestion(questionId)
        .subscribe((question) => {
